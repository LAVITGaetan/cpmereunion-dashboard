...
exports.deleteSondage = (req, res) => {
    ...
    instance.delete(`/sondages/${req.params.id}`, {
        ...
        instance.delete(`/sondages/${req.params.id}/questions`, {
            headers: {
                'auth-token': token
            }
        })
            .then((response) => {
                console.log('Questions supprimées supprimé');
                req.flash('message', `Sondage supprimé`);
                res.redirect(`/sondages`)
            })
            .catch((error) => {
                console.log(error.message);
                req.flash('message', `Sondage supprimé mais une erreure est survenue pour les questions`);
                res.redirect(`/sondages`)
            })
}
